...
def main():
    ...
    if uploaded_file_queries:
        ...
        if comparison_mode == "Périodes Consécutives":
            ...
            months = options[selection]; n_end = anchor_date; n_start = get_start_of_month(anchor_date, months - 1)
            n1_end = n_start - timedelta(days=1); n1_start = get_start_of_month(n1_end, months - 1)
            periode_n_dates, periode_n1_dates = (n_start, n_end), (n1_start, n1_end); period_type = f"{selection} vs Précédent"
        else:
        ...
        if periode_n_dates:
            ...
            if metrics['total_clics_n'] == 0 and metrics['total_clics_n1'] == 0:
                ...
                chart_configs = {"global": {"title": "Trafic SEO Global", "metric_n": "total_clics_n", "metric_n1": "total_clics_n1"}, "marque": {"title": "Trafic SEO Marque", "metric_n": "clics_marque_n", "metric_n1": "clics_marque_n1"}, "hors_marque": {"title": "Trafic SEO Hors-Marque", "metric_n": "clics_hors_marque_n", "metric_n1": "clics_hors_marque_n1"}, "impressions": {"title": "Impressions SEO Marque", "metric_n": "impressions_marque_n", "metric_n1": "impressions_marque_n1"}}
                
                with st.expander("✏️ Personnaliser le titre du graphique d'évolution"):
                    default_title = f"Synthèse des Évolutions (%) - {period_type}"; st.session_state.chart_titles['evolution'] = st.text_input("Titre:", value=st.session_state.chart_titles.get('evolution', default_title), key="title_evolution")
                ...
                with st.expander("✏️ Personnaliser les titres des graphiques de répartition"):
                    default_title_n1 = f"Répartition Période N-1: {metrics['nom_periode_n1']}"; default_title_n = f"Répartition Période N: {metrics['nom_periode_n']}"; st.session_state.chart_titles['pie_n1'] = st.text_input("Titre N-1:", value=st.session_state.chart_titles.get('pie_n1', default_title_n1), key="title_pie_n1"); st.session_state.chart_titles['pie_n'] = st.text_input("Titre N:", value=st.session_state.chart_titles.get('pie_n', default_title_n), key="title_pie_n")
                ...
                if monthly_data is not None and len(monthly_data) > 1:
                    ...
                    monthly_chart_configs = {"global": {"title": "Trafic SEO Global", "metric_n": "total_clics", "metric_n1": "total_clics_n1"}, "marque": {"title": "Trafic SEO Marque", "metric_n": "clics_marque", "metric_n1": "clics_marque_n1"}, "hors_marque": {"title": "Trafic SEO Hors-Marque", "metric_n": "clics_hors_marque", "metric_n1": "clics_hors_marque_n1"}, "impressions": {"title": "Impressions SEO Marque", "metric_n": "impressions_marque", "metric_n1": "impressions_marque_n1"}}
