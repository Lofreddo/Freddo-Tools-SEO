...
# --- Fonctions de Création de Graphiques (Mises à jour pour accepter les titres) ---
def create_evolution_chart(metrics, style_options, custom_title):
    COLORS, evolutions = get_colors(), []
    # ... (logique de calcul des évolutions inchangée) ...
    ...
    fig.update_layout(title=custom_title, font=dict(family=style_options['font_family'], size=style_options['axis_font_size']), title_font_size=style_options['title_font_size'], height=500, plot_bgcolor='white', yaxis=dict(zeroline=True, zerolinewidth=2, zerolinecolor='black'))
    return fig

def create_pie_charts(metrics, style_options, custom_title_n1, custom_title_n):
    ...
    for i, period in enumerate(['n1', 'n']):
        ...
        if total > 0:
            ...
            title = custom_title_n1 if period == 'n1' else custom_title_n
            fig.update_layout(title=title, height=450, font=dict(family=style_options['font_family']), title_font_size=style_options['title_font_size'])
            figs.append(fig)
...
def create_monthly_breakdown_chart(monthly_data, style_options, config, custom_title):
    COLORS = get_colors()
    fig = go.Figure()
    ...
    fig.update_layout(title=custom_title, xaxis_title="Mois", yaxis_title=config['yaxis_title'], barmode='group', font=dict(family=style_options['font_family'], size=style_options['axis_font_size']), title_font_size=style_options['title_font_size'], height=500, plot_bgcolor='white', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig

def show_chart_customization(period_type, metrics):
    """Affiche l'interface de personnalisation des graphiques."""
    with st.expander("🎨 Personnalisation Globale", expanded=False):
        # ... (logique de personnalisation des couleurs et polices) ...
        if st.button("✍️ Réinitialiser les titres des graphiques"):
            if 'chart_titles' in st.session_state:
                del st.session_state['chart_titles']
            st.rerun()
...
def main():
    ...
    col1, col2 = st.columns(2); uploaded_file_queries = col1.file_uploader("1. Fichier 'Requêtes' (Obligatoire)", type=['xlsx', 'xls']); uploaded_file_pages = col2.file_uploader("2. Fichier 'Pages' (Recommandé)", type=['xlsx', 'xls'])
    ...
    if uploaded_file_queries:
        ...
        # ... (logique de sélection de période inchangée) ...
        ...
        if periode_n_dates:
            ...
            # Initialise les titres par défaut si nécessaire
            if 'chart_titles' not in st.session_state: st.session_state.chart_titles = {}
            ...
            show_chart_customization(period_type, metrics)
            ...
            if metrics['total_clics_n'] == 0 and metrics['total_clics_n1'] == 0:
                ...
                chart_configs = {"global": {"title": "Trafic SEO Global", "yaxis_title": "Clics", "metric_n": "total_clics_n", "metric_n1": "total_clics_n1", "color": get_colors()['global_seo']}, "marque": {"title": "Trafic SEO Marque", "yaxis_title": "Clics", "metric_n": "clics_marque_n", "metric_n1": "clics_marque_n1", "color": get_colors()['marque_clics']}, "hors_marque": {"title": "Trafic SEO Hors-Marque", "yaxis_title": "Clics", "metric_n": "clics_hors_marque_n", "metric_n1": "clics_hors_marque_n1", "color": get_colors()['hors_marque']}, "impressions": {"title": "Impressions SEO Marque", "yaxis_title": "Impressions", "metric_n": "impressions_marque_n", "metric_n1": "impressions_marque_n1", "color": get_colors()['impressions_marque']}}
                
                # --- Logique de personnalisation et d'affichage ---
                with st.expander("✏️ Personnaliser le titre du graphique d'évolution"):
                    default_title = f"Synthèse des Évolutions (%) - {period_type}"
                    st.session_state.chart_titles['evolution'] = st.text_input("Titre:", value=st.session_state.chart_titles.get('evolution', default_title), key="title_evolution")
                st.plotly_chart(create_evolution_chart(metrics, get_style_options(), st.session_state.chart_titles['evolution']), use_container_width=True)

                with st.expander("✏️ Personnaliser les titres des graphiques de répartition"):
                    default_title_n1 = f"Répartition Période N-1: {metrics['nom_periode_n1']}"; default_title_n = f"Répartition Période N: {metrics['nom_periode_n']}"
                    st.session_state.chart_titles['pie_n1'] = st.text_input("Titre N-1:", value=st.session_state.chart_titles.get('pie_n1', default_title_n1), key="title_pie_n1")
                    st.session_state.chart_titles['pie_n'] = st.text_input("Titre N:", value=st.session_state.chart_titles.get('pie_n', default_title_n), key="title_pie_n")
                ...
                for key, config in chart_configs.items():
                    with st.expander(f"✏️ Personnaliser le titre du graphique '{config['title']}'"):
                        default_title = f"{config['title']} ({config['yaxis_title']}) - {period_type}"
                        st.session_state.chart_titles[key] = st.text_input("Titre:", value=st.session_state.chart_titles.get(key, default_title), key=f"title_{key}")
                    st.plotly_chart(create_generic_bar_chart(metrics, get_style_options(), config, st.session_state.chart_titles[key]), use_container_width=True)
                ...
                if monthly_data is not None and len(monthly_data) > 1:
                    ...
                    monthly_chart_configs = {"global": {"title": "Trafic SEO Global", "yaxis_title": "Clics", "metric_n": "total_clics", "metric_n1": "total_clics_n1", "color": get_colors()['global_seo']}, "marque": {"title": "Trafic SEO Marque", "yaxis_title": "Clics", "metric_n": "clics_marque", "metric_n1": "clics_marque_n1", "color": get_colors()['marque_clics']}, "hors_marque": {"title": "Trafic SEO Hors-Marque", "yaxis_title": "Clics", "metric_n": "clics_hors_marque", "metric_n1": "clics_hors_marque_n1", "color": get_colors()['hors_marque']}, "impressions": {"title": "Impressions SEO Marque", "yaxis_title": "Impressions", "metric_n": "impressions_marque", "metric_n1": "impressions_marque_n1", "color": get_colors()['impressions_marque']}}
                    for key, config in monthly_chart_configs.items():
                        with st.expander(f"✏️ Personnaliser le titre du graphique '{config['title']} (Mensuel)'"):
                            default_title = f"{config['title']} (Évolution Mensuelle)"
                            st.session_state.chart_titles[f"monthly_{key}"] = st.text_input("Titre:", value=st.session_state.chart_titles.get(f"monthly_{key}", default_title), key=f"title_monthly_{key}")
                        st.plotly_chart(create_monthly_breakdown_chart(monthly_data, get_style_options(), config, st.session_state.chart_titles[f"monthly_{key}"]), use_container_width=True)
